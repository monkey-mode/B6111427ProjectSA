...
// was not loaded in eager-loading, or loaded but was not found.
func (e BookingEdges) BookOrErr() (*Bookingtype, error) {
	if e.loadedTypes[1] {
		if e.Book == nil {
			// The edge book was loaded in eager-loading,
			// but was not found.
			return nil, &NotFoundError{label: bookingtype.Label}
		}
		return e.Book, nil
	}
...
func (b *Booking) assignValues(values ...interface{}) error {
	...
	if len(values) == len(booking.ForeignKeys) {
		if value, ok := values[0].(*sql.NullInt64); !ok {
			return fmt.Errorf("unexpected type %T for edge-field BOOKINGTYPE_ID", value)
		} else if value.Valid {
			b.BOOKINGTYPE_ID = new(int)
			*b.BOOKINGTYPE_ID = int(value.Int64)
		}
...
// QueryBook queries the book edge of the Booking.
func (b *Booking) QueryBook() *BookingtypeQuery {
	return (&BookingClient{config: b.config}).QueryBook(b)
}
