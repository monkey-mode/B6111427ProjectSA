...
// The BookingtypeFunc type is an adapter to allow the use of ordinary
// function as Bookingtype mutator.
type BookingtypeFunc func(context.Context, *ent.BookingtypeMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f BookingtypeFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.BookingtypeMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.BookingtypeMutation", m)
	}
	return f(ctx, mv)
}
